...
Then install [PyTorch](https://pytorch.org/get-started/locally/) v2. Please change the CUDA version according to your system. My code:

```bash
pip3 install torch==2.0.0+cu118 torchvision==0.15.1+cu118 torchaudio==2.0.1 --index-url https://download.pytorch.org/whl/cu118
```

Next install MMCV. Please change the CUDA version according to your system. My code:

```bash
pip3 install mmcv -f https://download.openmmlab.com/mmcv/dist/cu118/torch2.0.0/index.html
```

> MMEngine is required by MMCV and thus automatically installed.

Finally install MMagic in the development mode:
