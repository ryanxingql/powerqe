# Welcome to PowerQE V4 Document
...
First create a "powerqev4" environment:

```bash
conda create -n powerqev4 python=3.8
conda activate powerqev4
```

Then install [PyTorch](https://pytorch.org/get-started/locally/) v2. My code:

```bash
conda install pytorch torchvision pytorch-cuda=11.8 -c pytorch -c nvidia
```

Next install MMEngine and MMCV. They are listed in "mmagic/requirements/mminstall.txt". My code:

```bash
pip3 install -r mmagic/requirements/mminstall.txt
```

> Using MIM to install MMEngine and MMCV is not recommended, because these two packages are always installed in a wrong environment by MIM and thus cannot be found.

Finally install MMagic in editable mode. My code:
