...
def cal_lq_out_psnr(gt_dir, lq_dir, img_names, args):
    if args.out_dir:
        img_infos = [
            dict(src=osp.join(gt_dir, img_name),
                 tar=osp.join(args.out_dir, img_name))
            for img_name in img_names
        ]
        ave_psnr, n_ignore = cal_imgdir_psnr(img_infos=img_infos,
...
def cal_videos_psnr(gt_dir, lq_dir, sub_dirs, args):
    ...
    if args.out_dir:
        ...
        for sub_dir in tqdm(sub_dirs, ncols=0):
            img_infos = [
                dict(src=osp.join(gt_dir, sub_dir['dir_name'], img_name),
                     tar=osp.join(args.out_dir, sub_dir['dir_name'], img_name))
                for img_name in sub_dir['img_names']
            ]
            psnr, n_ignore = cal_imgdir_psnr(img_infos=img_infos,
                                             silent=True,
                                             crop_boarder=args.crop_boarder)
            results.append(psnr)
