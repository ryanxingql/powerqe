...
def pad_and_add(x, y):
    ...
    if diff > 0:
        y_pads[2] = diff // 2
        y_pads[3] = diff - diff // 2
    elif diff < 0:
        x_pads[2] = (-diff) // 2
        x_pads[3] = (-diff) - (-diff) // 2

    # w
    diff = x.shape[3] - y.shape[3]
    if diff > 0:
        y_pads[0] = diff // 2
        y_pads[1] = diff - diff // 2
    elif diff < 0:
        x_pads[0] = (-diff) // 2
...
class Encoder(nn.Module):
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        ...
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc3 = pad_and_add(enc3, self.csff_enc3(encoder_outs[2]))
            enc3 = pad_and_add(enc3, self.csff_dec3(decoder_outs[2]))
...
class ORSNet(nn.Module):
    ...
    def __init__(self, n_feat, scale_orsnetfeats, kernel_size, reduction, act,
        ...
        self.orb1 = ORB(n_feat + scale_orsnetfeats, kernel_size, reduction,
                        act, bias, num_cab)
        self.orb2 = ORB(n_feat + scale_orsnetfeats, kernel_size, reduction,
                        act, bias, num_cab)
        self.orb3 = ORB(n_feat + scale_orsnetfeats, kernel_size, reduction,
                        act, bias, num_cab)
    ...
    def forward(self, x, encoder_outs, decoder_outs):
        ...
        x = self.orb2(x)
        x = pad_and_add(x, self.conv_enc2(self.up_enc1(encoder_outs[1])))
        x = pad_and_add(x, self.conv_dec2(self.up_dec1(decoder_outs[1])))

        x = self.orb3(x)
        x = pad_and_add(x, self.conv_enc3(self.up_enc2(encoder_outs[2])))
        x = pad_and_add(x, self.conv_dec3(self.up_dec2(decoder_outs[2])))
        return x
...
class MPRNet(BaseNet):
    ...
    def forward(self, x3_img):
        ...
        x2bot_samfeats, _ = self.sam12(res1_bot[0], x2bot_img)
        ...
        x3_samfeats, _ = self.sam23(res2[0], x3_img)
